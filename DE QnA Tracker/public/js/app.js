...
async function loadNotifications() {
  if (!currentUser) return;
  try {
    const { data, error } = await sb.from('collab_notifications').select('id,recipient_email,actor_email,actor_name,type,title,body,link_type,link_ref,metadata,is_read,read_at,created_at').eq('recipient_email', currentUser.email).order('created_at', { ascending:false }).limit(80);
    if (error) throw error;
...
// ═══════════════════════════════════════════════════════════════════════════
// TAB RESUME WATCHDOG
// Fixes a common production issue where Chrome pauses background tabs and
// Supabase/auth/realtime requests resume in a stale state. Without this, a
// page can remain stuck on a "Loading..." panel until the user presses F5.
// ═══════════════════════════════════════════════════════════════════════════
(function installTabResumeWatchdog(){
  if (window.__tabResumeWatchdogInstalled) return;
  window.__tabResumeWatchdogInstalled = true;
  ...
  function activeRouteId() {
    ...
    try { return normalizeRouteId(window.location.hash || localStorage.getItem(ROUTE_STORAGE_KEY)); } catch (_e) { return 'submit'; }
  }

  function hasStuckLoader(routeId) {
    try {
      const page = document.getElementById('page-' + routeId) || document.querySelector('.page.active');
      if (!page) return false;
      return !!page.querySelector('.loading, .comment-loading');
    } catch (_e) { return false; }
  }

  async function refreshCurrentViewAfterResume() {
    if (document.hidden || resumeRunning || !currentUser) return;
    const now = Date.now();
    if (now - lastResumeAt < 2500) return;
    lastResumeAt = now;
    resumeRunning = true;
    try {
      // Refresh Supabase auth first. A stale auth/session is the most common
      // reason data loads silently fail after a sleeping browser tab resumes.
      const { data, error } = await sb.auth.getSession();
      if (!error && data && data.session && data.session.user) currentUser = data.session.user;

      const route = activeRouteId();

      // Team Chat uses realtime channels. Reconnect and reload silently when
      // returning to the tab so messages do not stay stuck on Loading.
      if (route === 'collab') {
        try {
          if (COLLAB && COLLAB.currentChannelId && typeof subscribeChannelRealtime === 'function') {
            subscribeChannelRealtime(COLLAB.currentChannelId);
          }
          if (typeof loadCollabMessages === 'function') {
            await Promise.race([
              loadCollabMessages(false, { silent:true, reason:'resume', skipSeen:true }),
              new Promise(resolve => setTimeout(resolve, 8000))
            ]);
          }
        } catch (err) { console.warn('Resume refresh for Team Chat failed:', err); }
        return;
      }

      // If another page is stuck showing a loader, re-run the route renderer.
      // This is intentionally conservative: normal pages are left alone.
      if (hasStuckLoader(route) && typeof showPage === 'function') {
        showPage(route);
      }
    } finally {
      resumeRunning = false;
  ...
  function scheduleResumeRefresh() {
    ...
    resumeTimer = setTimeout(refreshCurrentViewAfterResume, 300);
  }
...
// ══════════════════════════════════════
// PRODUCTION READY v1.5 — notification coverage, deep links, follow-up sync, inline editing
// ══════════════════════════════════════
(function(){
  'use strict';
  const NOTIFICATION_SELECT = 'id,recipient_email,actor_email,actor_name,type,title,body,link_type,link_ref,metadata,is_read,read_at,created_at';
  const NOTIFICATION_TYPES = {
    QUESTION_ANSWERED:'Question answered', ANSWER_EDITED:'Answer edited', QUESTION_EDITED:'Question edited', USER_TAGGED:'You were mentioned',
    REVIEW_ASSIGNED:'Review assigned', REVIEWER_ACTION:'Reviewer update', STATUS_UPDATED:'Status updated', COMMENT_ADDED:'New follow-up',
    COMMENT_REPLIED:'New reply', FOLLOWUP_RESOLVED:'Follow-up resolved', ADMIN_ANNOUNCEMENT:'Admin announcement'
  };
  const qnaEditStamp = (value) => value ? `<span class="edited-label">edited ${fmtDateFull(value)}</span>` : '';
  const qnaIsOwnQuestion = q => !!(currentUser && q && String(q.submitter_email||'').toLowerCase() === String(currentUser.email||'').toLowerCase());
  const qnaCanEditAnswer = q => !!(currentUser && q && (isReviewer() || String(q.answered_by||'').toLowerCase().includes(String(currentUser.email||'').toLowerCase())));

  window.notificationTitle = function(type) { return NOTIFICATION_TYPES[type] || 'Notification'; };

  window.loadNotifications = async function() {
    if (!currentUser) return;
    try {
      const { data, error } = await sb.from('collab_notifications')
      ...
      collabOpenError('load notifications', err);
  ...
  window.renderNotifications = function() {
    const notifications = COLLAB.notifications || [];
    COLLAB.unread = notifications.filter(n => !n.is_read).length;
    const badge = document.getElementById('notification-badge');
    if (badge) { badge.textContent = COLLAB.unread > 99 ? '99+' : String(COLLAB.unread); badge.classList.toggle('visible', COLLAB.unread > 0); }
    const list = document.getElementById('notification-list');
    if (!list) return;
    if (!notifications.length) {
      list.innerHTML = '<div class="empty"><div class="empty-icon">🔔</div><h3>No notifications yet</h3><p>Mentions, answers, comments, reviewer updates, and admin announcements will appear here.</p></div>';
      return;
    }
    list.innerHTML = notifications.map(n => {
      const state = n.is_read ? 'read' : 'unread';
      const typeLabel = notificationTitle(n.type);
      return `<div class="notification-item ${state}" data-notification-id="${collabAttr(n.id)}" onclick="openNotification('${collabAttr(n.id)}')">
        <div class="notification-title"><span class="notification-type-chip">${collabHtml(typeLabel)}</span>${collabHtml(n.title || typeLabel)}</div>
        <div class="notification-body">${collabHtml(n.body || '')}</div>
        <div class="notification-time">${fmtDateFull(n.created_at)}${n.is_read ? ' · read' : ''}</div>
      </div>`;
    }).join('');
  };

  const oldSubscribeNotifications = window.subscribeNotifications;
  window.subscribeNotifications = function() {
    ...
    try {
      COLLAB.subs.notifications = sb.channel('collab_notifications_' + currentUser.email)
        ...
        .on('postgres_changes', { event:'UPDATE', schema:'public', table:'collab_notifications', filter:`recipient_email=eq.${currentUser.email}` }, payload => {
          const idx = COLLAB.notifications.findIndex(n => n.id === payload.new.id);
          if (idx > -1) COLLAB.notifications[idx] = payload.new;
          renderNotifications();
        })
        .subscribe();
    } catch (err) { collabOpenError('notification subscribe', err); if (typeof oldSubscribeNotifications === 'function') oldSubscribeNotifications(); }
  };
  ...
  window.openNotification = async function(id) {
    ...
    document.getElementById('notification-panel')?.classList.remove('open');
    const meta = n.metadata || {};
    if (n.link_type === 'channel' && n.link_ref) {
      showPage('collab');
      COLLAB.currentChannelId = n.link_ref;
      localStorage.setItem('collabCurrentChannelId', n.link_ref);
      await loadCollaborationDashboard();
      if (meta.message_id) setTimeout(() => qnaHighlight(`[data-message-id="${CSS.escape(meta.message_id)}"]`), 450);
      return;
    }
    if (n.link_type === 'question' || meta.question_uuid || n.link_ref) {
      await openQuestionDeepLink(n.link_ref || meta.question_uuid, meta);
    }
  };

  async function openQuestionDeepLink(questionId, meta={}) {
    const ref = String(questionId || meta.question_uuid || '').trim();
    if (isReviewer()) {
      showPage('review');
      await loadReviewData();
      const q = (allQuestions || []).find(x => x.id === ref || x.question_id === meta.question_id || x.task_id === meta.bill_id);
      if (q) {
        openModal(q.id);
        setTimeout(async () => {
          if (meta.comment_id) await loadModalComments(q.id);
          if (meta.comment_id) qnaHighlight(`[data-comment-id="${CSS.escape(meta.comment_id)}"]`);
          else qnaHighlight('#modal-overlay .modal-content, #modal-overlay .modal');
        }, 650);
      }
      return;
    }
    const lookup = meta.bill_id || meta.question_id || ref;
    showPage('faq');
    setTimeout(() => {
      openAnsweredByRef(lookup);
      if (meta.comment_id) setTimeout(() => qnaHighlight(`[data-comment-id="${CSS.escape(meta.comment_id)}"]`), 800);
    }, 600);
  }

  function qnaHighlight(selector) {
    const el = document.querySelector(selector);
    if (!el) return;
    el.scrollIntoView({ behavior:'smooth', block:'center' });
    el.classList.add('qna-highlight-target');
    setTimeout(() => el.classList.remove('qna-highlight-target'), 2600);
  }

  async function resolveOpenFollowupsForReviewerReply(questionId) {
    if (!questionId || !currentUser || !isReviewer()) return;
    try {
      const now = new Date().toISOString();
      const { error } = await sb.from('question_comments')
        .update({ is_resolved:true, updated_at:now })
        .eq('question_id', questionId)
        .eq('is_reviewer_reply', false)
        .eq('is_resolved', false);
      if (error) throw error;
      if (commentCache[questionId]) {
        commentCache[questionId] = commentCache[questionId].map(c => !c.is_reviewer_reply ? { ...c, is_resolved:true } : c);
        setQuestionCommentStats(questionId, commentCache[questionId]);
        updateFollowUpBadges(questionId);
        updateReviewNavBadge();
        updateStatsIfReviewLoaded();
      }
    } catch (err) { console.warn('Could not auto-resolve follow-ups:', err?.message || err); }
  }

  const previousAddComment = window.addComment || addComment;
  window.addComment = addComment = async function(questionId, text, parentId = null) {
    const data = await previousAddComment(questionId, text, parentId);
    if (!data) return data;
    if (isReviewer()) {
      await resolveOpenFollowupsForReviewerReply(questionId);
      await autoMarkQuestionAnsweredOnReviewerReply(questionId);
    }
    return data;
  };

  const previousSaveAnswer = window.saveAnswer || saveAnswer;
  window.saveAnswer = saveAnswer = async function() {
    const before = currentQ ? { ...currentQ } : null;
    const oldAnswer = before?.answer || '';
    const answerEl = document.getElementById('m-answer');
    const newAnswer = answerEl ? answerEl.value.trim() : oldAnswer;
    const changedAnswer = before && oldAnswer && newAnswer && oldAnswer !== newAnswer;
    if (changedAnswer) {
      try { await sb.from('questions').update({ answer_edited_at:new Date().toISOString(), answer_edited_by:currentUser.email }).eq('id', before.id); } catch (_e) {}
    }
    const result = await previousSaveAnswer();
    if (before?.id && isReviewer()) await resolveOpenFollowupsForReviewerReply(before.id);
    const after = before ? ((allQuestions || []).find(q => q.id === before.id) || currentQ || before) : null;
    if (before && after && before.submitter_email && before.submitter_email !== currentUser?.email && changedAnswer) {
      await createNotifications([{ recipient_email:before.submitter_email, actor_email:currentUser.email, actor_name:collabCurrentName(), type:'ANSWER_EDITED', title:'Answer updated', body:`The answer for ${before.question_id || before.task_id} was updated.`, link_type:'question', link_ref:before.id, metadata:{ question_uuid:before.id, question_id:before.question_id, bill_id:before.task_id } }]);
    }
    return result;
  };

  window.openEditQuestionModal = function(questionId) {
    const q = [...(allQData||[]), ...(allQuestions||[]), ...(faqQuestions||[])].find(x => x.id === questionId);
    if (!q || !qnaIsOwnQuestion(q)) { toast('You can edit only your own question.', 'error'); return; }
    document.getElementById('modal-title').textContent = `Edit ${q.question_id || 'question'}`;
    document.getElementById('modal-meta').innerHTML = `<span class="pill-muted">Bill ID ${escHtml(q.task_id || '')}</span>`;
    document.getElementById('modal-body').innerHTML = `<div class="form-group"><label class="form-label">Question</label><textarea class="form-textarea" id="edit-question-text" maxlength="2000">${escHtml(q.question || '')}</textarea><div class="form-hint">Editing will notify reviewers/admins and show an edited label.</div></div>`;
    document.getElementById('modal-footer').innerHTML = `<button class="btn btn-outline" onclick="closeModal()">Cancel</button><button class="btn btn-success" onclick="saveQuestionEdit('${q.id}')">Save edit</button>`;
    document.getElementById('modal-overlay').classList.add('open');
  };

  window.saveQuestionEdit = async function(questionId) {
    const q = [...(allQData||[]), ...(allQuestions||[]), ...(faqQuestions||[])].find(x => x.id === questionId);
    const text = (document.getElementById('edit-question-text')?.value || '').trim();
    if (!q || !qnaIsOwnQuestion(q)) { toast('You can edit only your own question.', 'error'); return; }
    if (text.length < 5) { toast('Question is too short.', 'error'); return; }
    try {
      const payload = { question:text, question_edited_at:new Date().toISOString(), question_edited_by:currentUser.email };
      const { data, error } = await sb.from('questions').update(payload).eq('id', questionId).eq('submitter_email', currentUser.email).select(AppAPI.QUESTION_FIELDS).single();
      if (error) throw error;
      [allQData, allQuestions, adminQuestions, faqQuestions].forEach(arr => { const i = (arr||[]).findIndex(x => x.id === questionId); if (i > -1) arr[i] = normalizeQuestionRecord(data || { ...arr[i], ...payload }); });
      await notifyReviewersForQuestionEdit(data || { ...q, ...payload });
      closeModal();
      if (document.getElementById('page-allq')?.classList.contains('active')) renderAllQ();
      if (document.getElementById('page-review')?.classList.contains('active')) renderReviewTable();
      toast('✓ Question updated', 'success');
    } catch (err) { toast('Could not edit question: ' + (err.message || err), 'error'); }
  };

  async function notifyReviewersForQuestionEdit(q) {
    await loadCollabUsers();
    const recipients = new Set((allowedReviewers || []).filter(Boolean));
    (COLLAB.users || []).forEach(u => { if (['reviewer','admin','primary_admin','reviewer_admin'].includes(String(u.role||'').toLowerCase())) recipients.add(u.email); });
    await createNotifications([...recipients].map(email => ({ recipient_email:email, actor_email:currentUser.email, actor_name:collabCurrentName(), type:'QUESTION_EDITED', title:'Question edited', body:`${q.question_id || q.task_id || 'A question'} was edited by the submitter.`, link_type:'question', link_ref:q.id, metadata:{ question_uuid:q.id, question_id:q.question_id, bill_id:q.task_id } })));
  }

  const oldRenderAllQ = window.renderAllQ || renderAllQ;
  window.renderAllQ = renderAllQ = function() {
    oldRenderAllQ();
    document.querySelectorAll('#allq-table-wrap tr[data-task-id]').forEach(tr => {
      const q = (allQData || []).find(x => x.task_id === tr.dataset.taskId);
      if (!q) return;
      const qCell = tr.querySelector('.td-q');
      if (qCell && q.question_edited_at && !qCell.querySelector('.question-edited-label')) qCell.insertAdjacentHTML('beforeend', `<div class="question-edited-label">edited ${fmtDateFull(q.question_edited_at)}</div>`);
      if (qCell && qnaIsOwnQuestion(q) && !qCell.querySelector('.edit-own-question-btn')) qCell.insertAdjacentHTML('beforeend', `<div class="inline-edit-actions"><button class="btn btn-outline btn-sm edit-own-question-btn" onclick="event.stopPropagation();openEditQuestionModal('${escAttr(q.id)}')">Edit question</button></div>`);
    });
  };

  const oldRenderFaq = window.renderFaq || renderFaq;
  window.renderFaq = renderFaq = function() {
    oldRenderFaq();
    document.querySelectorAll('.faq-card[data-question-id]').forEach(card => {
      const q = (faqQuestions || []).find(x => x.id === card.dataset.questionId);
      if (!q) return;
      const questionEl = card.querySelector('.faq-question');
      const answerEl = card.querySelector('.faq-answer-preview');
      if (questionEl && q.question_edited_at && !questionEl.querySelector('.question-edited-label')) questionEl.insertAdjacentHTML('beforeend', `<div class="question-edited-label">edited ${fmtDateFull(q.question_edited_at)}</div>`);
      if (answerEl && q.answer_edited_at && !answerEl.querySelector('.answer-edited-label')) answerEl.insertAdjacentHTML('afterend', `<div class="answer-edited-label">edited ${fmtDateFull(q.answer_edited_at)}</div>`);
    });
  };

  const oldOpenModal = window.openModal || openModal;
  window.openModal = openModal = function(id) {
    const result = oldOpenModal(id);
    setTimeout(() => {
      const q = currentQ;
      const questionBox = document.querySelector('#modal-body .detail-box');
      if (q?.question_edited_at && questionBox && !questionBox.querySelector('.question-edited-label')) questionBox.insertAdjacentHTML('beforeend', `<div class="question-edited-label">edited ${fmtDateFull(q.question_edited_at)}</div>`);
      const answerTa = document.getElementById('m-answer');
      if (q?.answer_edited_at && answerTa && !document.getElementById('modal-answer-edited-label')) answerTa.insertAdjacentHTML('afterend', `<div id="modal-answer-edited-label" class="answer-edited-label">edited ${fmtDateFull(q.answer_edited_at)}</div>`);
    }, 0);
    return result;
  };
})();
