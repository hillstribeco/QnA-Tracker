...
2. Paste the entire contents of `supabase/migrations/0000_baseline.sql`.
3. Click **Run**.
4. Wait for "Success".
...
### Step 2b — Run production and attachment migrations

Run these two files after the baseline:

1. `supabase/migrations/0001_production_hardening.sql`
2. `supabase/migrations/0002_attachments_storage.sql`

The second migration creates the `qna-attachments` Storage bucket and policies required for image/file uploads in questions, answers, follow-ups, and Team Chat.

### Step 3 — Replace your file structure (10 min)

1. Take your existing `index.html` and copy it to the new `public/` folder.
...
If you're currently deploying the root folder, update Netlify's **Publish directory** to `public`.
