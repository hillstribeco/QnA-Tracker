...
This file records exactly what was changed when `index_production_refactored_qa_hardened.html` was cleaned up into the new `frontend/index.html`.
...
The actual values now live in `frontend/config.js`, which is loaded before the rest of the JavaScript runs.
