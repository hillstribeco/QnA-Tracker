...
4. Go to **SQL Editor** → **New query** → paste the **entire contents** of `database/00_baseline.sql` → click **Run**.
5. Confirm: go to **Table Editor** in the left sidebar — you should see ~17 tables.
...
> **Don't commit secrets!** Make sure `frontend/config.js` is either using *only* the Supabase public anon key (safe to commit), or is replaced at deploy time. See [`SECURITY.md`](SECURITY.md) for details.

---

## Step 4 — Wire up the frontend credentials

Open `frontend/config.js` and paste in the values from Step 1:
...
- **Frontend changes:** edit, commit, push → Netlify auto-deploys in ~30 sec.
- **Database changes:** add a file to `database/migrations/` → run it once in Supabase SQL Editor.
