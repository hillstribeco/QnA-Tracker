...
All tables live in the `public` schema in your Supabase PostgreSQL database. The complete creation script is `supabase/migrations/0000_baseline.sql`.
...
### `qna-attachments` bucket

Created by `supabase/migrations/0002_attachments_storage.sql`. The frontend uploads question, answer, follow-up, and Team Chat files into this public bucket, then stores only the resulting public URL in the existing text field as `[Attachment: file-name](https://...)`.

Storage policies allow public read access for rendered previews and authenticated insert/update/delete access only under the signed-in user's auth ID folder.
